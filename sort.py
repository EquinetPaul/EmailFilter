...
def write_new_list(listEmail):
    ...
    for email in listEmail: # pour chacune des emails dans la liste
        list_to_add += email + "\n" # on ajoute à la chaine de caractère l'adresse email
    fileListEmail = open("newList.txt", "w") # on ouvre la nouvelle liste
    fileListEmail.write(list_to_add + "\n") # on écrit dans la nouvelle liste
...
know_domain = ['gmail','free','outlook','hotmail','icloud','neuf','orange','yahoo','laposte']

print('Charging ' + str(len(listEmail)) + ' Emails...')

# fonctions principales du programme, à mettre en commentaire si vous ne voulez pas l'executer
...
listEmail = delete_iphone_password(listEmail) # supprimer les mots de passe iphone
write_new_list(listEmail) # ecrire les nouvelles entrées
make_different_files_for_domain(listEmail,know_domain) # créer des fichiers contenant les emails en fonction des différents domaines
